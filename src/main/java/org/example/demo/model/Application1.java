...
import org.example.demo.InfluencerDAO;
...
public class Application1 {
    ...
    private final Map<String, Integer> initialFollowersMap;
    ...
    public Application1(InputDevice inputDevice, OutputDevice outputDevice) {
        ...
        loadData();
        this.initialFollowersMap = new HashMap<>();
        this.influencerDAO = new InfluencerDAO(DatabaseConnection.getConnection());
    }
    ...
    public int getInitialFollowers(String influencerId) {
        return initialFollowersMap.getOrDefault(influencerId, 0); // Default to 0 if not found
    }
    ...
    public Influencer getInfluencerByName(String name) {
        return influencerDAO.getInfluencerByName(name); // Delegate call to DAO
    }
    ...
    public String compareEngagement(int currentLikes1, int comments1, int currentLikes2, int comments2) {
        ...
        if (comparisonResult < 0) {
            return "Engagement for GA1 is less than GA2.";
        } else if (comparisonResult > 0) {
            return "Engagement for GA1 is greater than GA2.";
        } else {
            return "GA1 and GA2 have the same engagement.";
        }
    }

    public void compareFollowerGrowth() {
        CompletableFuture.runAsync(() -> {
            try {
    ...
    public void updateSortCriterion(int choice) throws InvalidNumberException {
        ...
        // Perform sorting in a background thread
        CompletableFuture.runAsync(() -> {
            synchronized (this) {
                influencers.sort(currentComparator); // Synchronize access if `influencers` can be accessed by multiple threads
            }
            outputDevice.displayMessage("Influencers sorted using the selected criterion.");
        });
    }
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    viewInfluencerDetails1(selectedInfluencer);
                    break;
                case 5:
                    return;
                default:
    ...
    public void viewInfluencerDetails1(Influencer influencer) {
    ...
    public void displayCampaignsAndPostsForInfluencer(Influencer influencer) {
        boolean influencerHasPosts = false;
    ...
    public void editPostForInfluencer(Influencer influencer, String postId) throws IOException, InvalidNumberException {
    ...
    public void compareEngagement1() throws InvalidNumberException {
